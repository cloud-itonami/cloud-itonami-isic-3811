(ns wastecollect.operation
  "OperationActor — one schedule/manifest/report/dispute operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (WasteDispatch-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the WasteDispatchGovernor (:govern)
  ...
  decision to a human reviewer (dispatch coordinator / dispute officer).
  The reviewer resumes with `{:approval {:status :approved}}` (or
  `:rejected`)."
...
(defn build
  ...
  `wastecollect.store/Store`).
  opts:
    :advisor      — a `wastecollect.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WasteDispatch-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; WasteDispatchGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:dispute? verdict) :dispute-request
                                          (:bulk? verdict) :bulk-volume
                                          :else :low-confidence))
                        :phase ph
      ...
      ;; (dispatch coordinator / dispute officer) resumes with :approval.
      ;; Then route commit/hold.
